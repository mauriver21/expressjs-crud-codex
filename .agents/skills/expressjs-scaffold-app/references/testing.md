...
`src/tests/globalSetup.ts` runs before the suite. It is deliberately gated by `TEST_DATABASE_SETUP=true` so a fresh scaffold can run the health test without a database. After a schema and migration exist, enable it in `.env.test`; it connects to the isolated test database, applies `./drizzle`, and disconnects.
